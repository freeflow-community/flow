...
// Slack behind it (#545 acceptance). Every route, the normalizer, the rate
...
//
//   node docs/qa/issue-545/fake-slack-connector.mjs [--port 8791] [--client http://127.0.0.1:5179]
//
...
//   - 8 s after start, one Events API message for C1 is delivered to the grant
...
const clientOrigin = args.client ?? 'http://127.0.0.1:5179';
...
const fetcher = async (url, request) => {
  ...
  switch (method) {
    ...
    case 'chat.postMessage': {
      ...
      if (!params.thread_ts) history.unshift(sent); // Slack's history now has it too
...
server.listen(port, '127.0.0.1', () => {
  console.log(JSON.stringify({ ready: true, origin: publicOrigin, credential, teamId: 'T1', userId: 'U1', channelId: 'C1', threadTs: TS(5) }));
});

// A message from bob lands through the Events API after 8 s: the browser must
// show it without a refetch (the connector routes it to the authorized grant).
setTimeout(() => {
  const live = message(400, { channel: 'C1', user: 'U2', text: 'live event from bob' });
  history.unshift(live); // it is in Slack's history from now on, like a real message
  const event = { type: 'event_callback', event_id: 'Ev-acceptance-1', team_id: 'T1', api_app_id: 'A1', authorizations: [{ user_id: 'U1', team_id: 'T1', is_bot: false }], event: live };
  const raw = Buffer.from(JSON.stringify(event));
  const timestamp = String(Math.floor(Date.now() / 1000));
  const signature = `v0=${createHmac('sha256', signingSecret).update(`v0:${timestamp}:`).update(raw).digest('hex')}`;
  connector.event(raw, timestamp, signature);
  console.log(JSON.stringify({ injected: 'live event from bob' }));
}, 8000);
