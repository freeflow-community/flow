...
| `FLOW_R2_BUCKET` | Bucket name (default `flow-files`). **One-time bucket setup**: create the bucket, then set a CORS policy — browsers preflight the presigned PUTs and CORS-check the 302'd GETs, so without it web uploads/downloads fail while native clients work fine: `aws s3api put-bucket-cors --bucket flow-files --endpoint-url $CLOUDFLARE_S3_ENDPOINT --cors-configuration '{"CORSRules":[{"AllowedOrigins":["https://app.freeflow.im","app://flow","https://flow.localhost","http://127.0.0.1:8787","http://localhost:8787","http://127.0.0.1:5173","http://localhost:5173"],"AllowedMethods":["GET","PUT","HEAD"],"AllowedHeaders":["content-type","range"],"ExposeHeaders":["content-length","content-type","content-range","etag"],"MaxAgeSeconds":3600}]}'` (propagates in ~1 min; the S3 endpoint's TLS itself provisions ~1 min after the first bucket exists). `https://flow.localhost` is the Android app's origin (`docs/design/ANDROID.md`); `app://flow` is the desktop app's origin (`docs/specs/desktop-electron.md`): without it, desktop uploads fail with "Failed to fetch" while everything else works. Read the live policy first with `get-bucket-cors` so you extend it rather than replace it. |
...
| `FLOW_PUSH_DRIVER` | `apns` — real iOS push over HTTP/2 to Apple. Unset/`dev` writes each push as a `simctl`-ready JSON file under `FLOW_PUSH_OUTBOX` and delivers nothing, which is right for local QA and wrong for production. Selecting `apns` without the three credentials below fails at boot rather than falling back. |
| `FLOW_APNS_KEY` | Base64 of the `.p8` APNs Auth Key (`base64 -i AuthKey_XXXXXXXXXX.p8`). One key per Apple Developer team, reusable across apps — **treat like a root credential**; anyone holding it can push to every app on the team. A raw PEM is accepted too, for the paste that skipped the base64 step. |
| `FLOW_APNS_KEY_ID` | The 10-char key id that names the `.p8` above (the `XXXXXXXXXX` in its filename). Rides in the provider JWT's `kid`. |
