...
| `FLOW_CORS_ORIGINS` | Origins allowed to call `/v1` cross-origin, for a client served from somewhere other than this process — the packaged Android client (`docs/design/ANDROID.md`), whose pages load from `capacitor://localhost` / `https://localhost`. Comma-separated, exact match, no credentials. Unset = no CORS layer at all, which is right for every web-only deployment: the web client is served by this same process and never needs it. |
