...
- Google sign-in on macOS/iOS goes through the **browser handoff**, not a native
  ...
  The iOS sheet runs **ephemeral** since #279, so it never inherits the
  device's Google or Flow session — the price of always being able to choose
  the account. macOS deliberately keeps the shared session: it opens the
  user's own browser, where being already signed in is the point. Not a gap.
