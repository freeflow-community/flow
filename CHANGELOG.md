...
- **Thread titles naming their parent conversation** (#417) ships on iOS and on
  web below the `md` breakpoint, and deliberately not on macOS or desktop web,
  by operator ruling. The split is layout, not platform: where the thread is a
  side panel with its channel open beside it, the title would only repeat what
  the screen already says; where the thread covers the channel (phone, and web
  below `md`), nothing else names the conversation. macOS has no narrow layout
  to qualify. The rule lives in `lib/channelTitle.ts` and on the `Channel`
  model, so a desktop surface that ever loses the side-by-side layout has it
  already. Not a gap to close.
