...
const report = { id: '01900000-0000-7000-a000-000000000001', channelId: 'channel', name: 'Flow vs Slack' } as ArtifactDTO;
...
describe('report delivery', () => {
  ...
  it('defaults inline reports to Markdown and separates the display name from the filename', () => {
    expect(inlineArtifactFile('Flow vs Slack')).toEqual({ filename: 'Flow vs Slack.md', mime: 'text/markdown' });
  ...
  it('posts a persistent artifact reference in the source thread with a stable message key', async () => {
    ...
    expect(sendMessage.mock.calls[0]?.slice(0, 4)).toEqual([
      'channel', `[Open report: Flow vs Slack](flow-artifact:${report.id})`, 'thread', undefined,
    ]);
