...
export class AgentBridge {
  ...
  private buildSystemPrompt(msg: MessageDTO, mcp: boolean): string {
    ...
    const lines = [
      ...
      mcp
        ? 'You have Flow MCP tools (send_message, react, upload_file, search_history); messages sent with send_message deliver immediately. Your final response text is ALSO posted to the conversation — if you already replied via send_message, keep the final text short or empty.'
