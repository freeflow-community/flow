...
| `list_artifacts` | Find saved reports in the current channel before revising them. |
| `deliver_artifact` | Retry a saved report's card delivery without creating a duplicate. |
...
- **Artifact** (`create_artifact`) — for a substantial deliverable: a report,
  ...
  Markdown reports render with tables on web and native report viewers. Web
  and Mac also support images, video, PDF, and HTML; iPhone report viewing
  currently supports Markdown/text. Use a private channel for private reports.
