...
- `[macos]` `[ios]` A delivered reply's row now re-renders when its optimistic
  copy is swapped for the server twin. Rows carry `.id(message.clientMsgId)`
  (#312) but `ForEach` still keyed elements on `message.id`, so the swap read as
  a delete + insert whose two views claimed one `.id()` — the leaving pending
  view won and the "sending" spinner stayed up over a message everyone else
  could see (#328). Keying the `ForEach` on `clientMsgId` too makes it one
  element with a changed value.
